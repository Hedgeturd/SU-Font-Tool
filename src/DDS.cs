...
namespace SUFontTool {
    public static class DDS {
        ...
        public static List<TextureEntry> textures = XML.textures;
        ...
        public static void Process() {
            ...
            foreach(TextureEntry texture in textures) {
                Console.WriteLine("Please provide the path of " + texture.Name + ".");
                string ddsPath = Console.ReadLine().Replace("\"", "");

                if (ddsPath != null && Path.GetFileNameWithoutExtension(ddsPath) != null) {
                    int index = textures.FindIndex(0, texCount, texture => texture.Name == Path.GetFileNameWithoutExtension(ddsPath));
        ...
        public static void ReadDDS(string filePath, int extTexIndex) {
            ...
            foreach (var chara in characters) {
                ...
                if (textureIndex == extTexIndex) {
                    string ConverseID = characters[spriteIndex].CharacterID.ToString();
                    int p1_x = (int)characters[spriteIndex].TopLeft.X;
                    int p1_y = (int)characters[spriteIndex].TopLeft.Y;
                    int p2_x = (int)characters[spriteIndex].BottomRight.X;
                    int p2_y = (int)characters[spriteIndex].BottomRight.Y;
